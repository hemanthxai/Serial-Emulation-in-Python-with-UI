...
#     pres=random.randint(1000,1040)
#     waterlev=random.randint(0,20)
...
# Update environment variable if in WSL
...
ser_temp = serial.Serial('/dev/pts/8', 9600, timeout=1)
ser_pres = serial.Serial('/dev/pts/11', 9600, timeout=1)
ser_water = serial.Serial('/dev/pts/12', 9600, timeout=1)
sensor_values = {
    "Temperature": "",
    "Pressure": "",
    "Water Level": "",
...
def read_sensor_data():
    """Read and parse sensor data from the serial ports."""
    global output_text
    try:
        if ser_temp.in_waiting:
            temp_data = ser_temp.readline().decode('utf-8').strip()
            sensor_values["Temperature"] = temp_data
        if ser_pres.in_waiting:
            pres_data = ser_pres.readline().decode('utf-8').strip()
            sensor_values["Pressure"] = pres_data
        if ser_water.in_waiting:
            water_data = ser_water.readline().decode('utf-8').strip()
            sensor_values["Water Level"] = water_data
    except Exception as e:
        output_text = f"Error reading sensor data: {e}"

def update_sensor_value(sensor_name:str):
    """Update the sensor value by reading from the serial port."""
    global output_text
    try:
        if sensor_name == "Temperature" and ser_temp.in_waiting:
            sensor_values[sensor_name] = ser_temp.readline().decode('utf-8').strip()
        elif sensor_name == "Pressure" and ser_pres.in_waiting:
            sensor_values[sensor_name] = ser_pres.readline().decode('utf-8').strip()
        elif sensor_name == "Water Level" and ser_water.in_waiting:
            sensor_values[sensor_name] = ser_water.readline().decode('utf-8').strip()
    except Exception as e:
        output_text = f"Error updating {sensor_name} value: {e}"
    ...
    for label in active.copy().keys():
        ...
        if imgui.button(f"Update {label}"):
            try:
                ...
                output_text = f"Error updating {label}: {e}"
    imgui.end()

    read_sensor_data()  # Read from serial ports
